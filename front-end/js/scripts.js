...
function myMove() { // move and pause scroller
  ...
  function frame() {
    if (slider == 196000) { // end behavior
      progSpot2 = 0;
      offMove();
      clearInterval(id);
    } else if (audioElement.paused){ // pause behavior
      timerThing = audioElement.currentTime;
      timerThing2 = timerThing.toFixed(0);
      progSpot = (timerThing2*1000)/140
      progSpot2 = progSpot.toFixed(0);
      //console.log(progSpot2);
      clearInterval(id);
    } else if (progSpot2 != 0 && progSpot2 < 1400 && slider <= 196000){ // post-pause or scroll-spot behavior
      pos++;
...
function home() {
  ...
  document.getElementById("infoContainer").innerHTML = '<h2>Welcome to The Illusionist roadmap!</h2>This song features seven different auditory illusions. Hover over the audio waveform to find the illusions, and click to learn more about that specific illusion.'; // update this to match home text!
}
